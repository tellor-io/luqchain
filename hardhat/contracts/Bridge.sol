...
contract Bridge {
    ...
    function verifyResultWithRoot(
        ...
        // Verify proof
        require(
            verifyProof(moduleStateRoot, version, key, dataHash, merklePaths),
    ...
    function relayAndVerify(
        ...
        verifyData = abi.encode(key, blockHeight, report, version, merklePaths);
    ...
    function verifyOracleData(
        bytes memory key,
        uint256 blockHeight,
        ReportCodec.Report calldata result,
        uint256 version,
        IAVLData[] calldata merklePaths
    ) public view returns (ReportCodec.Report memory) {
        bytes32 stateRoot = blockDetails[blockHeight].moduleState;
        require(stateRoot != bytes32(uint256(0)), "NO_ORACLE_ROOT_STATE_DATA");

        return
            verifyResultWithRoot(key, stateRoot, version, result, merklePaths);
    }
